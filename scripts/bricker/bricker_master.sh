...
# call as     bash bricker.sh [number of source bricks] [rows to generate] [bricks per row] [random dimming factor] [rflip] [rflop] [display]"
#       i.e.  bash bricker.sh 10 31 20 90
...
if [ -z "$4" ] ; then
	...
	echo "     bash bricker.sh [number of source bricks] [rows to generate] [bricks per row] [random dimming factor]"
	echo "i.e. bash bricker.sh 10 31 20 90"
else
	# process args
	pix=$1
	rows=$2
	...
	do
		...
		do
			...
			convert $(rint $pix).png $rfl tmp_.png
